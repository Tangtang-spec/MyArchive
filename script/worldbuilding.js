...
function listenToWorldbuilding(projectId) {
    ...
    unsubscribeWbListener = onSnapshot(q, (snapshot) => {
        ...
        console.error("Error fetching worldbuilding items:", error);
        wbContainer.innerHTML = `<div class="empty-state">เกิดข้อผิดพลาดในการโหลดข้อมูล Worldbuilding</div>`;
    });
}

// --- 4. Render and Filter Data ---
function renderWbItems() {
    ...
    const counts = {
        ...
        faction: rawWbItems.filter(i => i.category === "faction").length,
    ...
    filterContainer.querySelectorAll(".tab-btn").forEach(btn => {
        ...
        const labels = {
            ...
            faction: `🛡️ กลุ่ม/หน่วยงาน (${counts.faction})`,
...
function createWbCard(item) {
    ...
    card.style.cursor = "pointer"; // เพิ่มตัวชี้ cursor ให้รู้ว่ากดได้
    ...
    const catMap = {
        ...
        faction: { label: "🛡️ กลุ่ม / องค์กร / หน่วยงาน", class: "supporting" },
    ...
    card.innerHTML = `
        ...
        <div class="char-body">
            ...
            ${item.content ? `<p class="wb-content-preview">${escapeHtml(item.content)}</p>` : ''}
        </div>
    ...
    // คลิกที่การ์ดเพื่อไปยังหน้ารายละเอียด
    card.addEventListener("click", (e) => {
        if (e.target.closest(".btn-action")) return;
        window.location.href = `worldbuilding-detail.html?id=${item.id}&projectId=${currentProjectId}`;
    });

    card.querySelector(".btn-edit-wb").addEventListener("click", (e) => {
        e.stopPropagation();
        openWbModal(item);
    });

    card.querySelector(".btn-delete-wb").addEventListener("click", (e) => {
        e.stopPropagation();
        deleteWbItem(item.id, item.title);
    });
